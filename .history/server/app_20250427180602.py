...
def logout():
    try:
        # Clear the JWT token from the client (this is handled on the client-side)
        # If using cookies, you would clear the cookie, e.g., by setting an expired cookie
        response = jsonify({"message": "Logout successful"})
        
        # If JWT token is stored in a cookie, we can set it to expire immediately:
        response.delete_cookie('access_token')  # Modify according to your cookie name
