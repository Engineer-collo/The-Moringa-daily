...
@api.route('/content/<int:content_id>/approve', methods=['POST'])
@jwt_required()
...
# POST flag content
@api.route('/content/<int:content_id>/flag', methods=['POST'])
@jwt_required()
...
# POST like content
@api.route('/content/<int:content_id>/like', methods=['POST'])
@jwt_required()
def like_content(content_id):
    content = Content.query.get(content_id)
    if content:
        content.likes = (content.likes or 0) + 1
        db.session.commit()
        return jsonify({"message": "Content liked"}), 200
    return jsonify({"error": "Content not found"}), 404

# POST dislike content
@api.route('/content/<int:content_id>/dislike', methods=['POST'])
@jwt_required()
def dislike_content(content_id):
    content = Content.query.get(content_id)
    if content:
        content.dislikes = (content.dislikes or 0) + 1
        db.session.commit()
        return jsonify({"message": "Content disliked"}), 200
    return jsonify({"error": "Content not found"}), 404

# POST comment on content
@api.route('/content/<int:content_id>/comment', methods=['POST'])
@jwt_required()
def comment_content(content_id):
    ...
    # Optionally, you can return updated content with comments
