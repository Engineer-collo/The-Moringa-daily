...
# Add the new route under your `resources_bp` blueprint
...
def flag_content(content_id):
    ...
    # Find the content to be flagged
    content = Content.query.get(content_id)
    if not content:
        return jsonify({"error": "Content not found"}), 404
    
    # Check if the content has already been flagged by the current user
    flag_exists = Flag.query.filter_by(user_id=current_user_id, content_id=content_id).first()
    if flag_exists:
        return jsonify({"message": "Content already flagged"}), 400
    
    # Create a new flag entry
    flag = Flag(user_id=current_user_id, content_id=content_id)
    db.session.add(flag)
    db.session.commit()
    
    return jsonify({"message": "Content flagged successfully"}), 201
...
def approve_content(content_id):
    ...
    if not admin_user or admin_user.role != 'admin':
        return jsonify({"error": "Unauthorized access"}), 403  # Only admins can approve content

    content = Content.query.get_or_404(content_id)  # Fetch the content by ID
    if content.approved:
        return jsonify({"message": "Content already approved"}), 400  # If already approved

    content.approved = True  # Set the content as approved
    db.session.commit()  # Commit changes to the database

    return jsonify(content.to_dict()), 200  # Return the approved content
