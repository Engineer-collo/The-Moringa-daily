...
@jwt_required(optional=True)  # optional=True if you want even non-logged-in users to see
def get_latest_content():
    latest_content = Content.query.order_by(Content.created_at.desc()).first()
    ...
    if latest_content:
        return jsonify({
            ...
            'category': latest_content.category.name if latest_content.category else None,
            'author': latest_content.author.username if latest_content.author else None,
        })
    else:
        return jsonify({"message": "No content found"}), 200  # Changing the response to 200 (OK)
