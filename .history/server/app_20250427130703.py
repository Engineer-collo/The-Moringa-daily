...
def get_user_data():
    try:
        ...
        if user:
            return jsonify(user.to_dict()), 200  # Assuming `to_dict()` method converts user data to JSON format
        else:
...
@resources_bp.route('/admin/user', methods=['GET'])
@jwt_required()  # Ensures the user must be authenticated
def get_user_data_for_admin():
    try:
        current_user_id = get_jwt_identity()  # Get the current user's ID from the JWT token
        ...
        # Fetch all users or specific user data for admin
